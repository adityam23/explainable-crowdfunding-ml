...
df = df.drop(
    columns=["id", "name", "location", "converted_pledged_amount", "usd_pledged"], errors="ignore"
)
...
if "blurb" in categorical:
    categorical.remove("blurb")
...
text_features = [
    col for col in X_df.columns if col in tf_vectorizer.get_feature_names_out()
] + other_text_cols
...
X_train_text, X_test_text, y_train, y_test = train_test_split(
    X_text_df, y, test_size=0.2, stratify=y, random_state=42
)

# ### MODEL 1 - Text only Model
trainer_text = CrowdfundingTrainer(
    model_type="xgb",
    params={
        "eval_metric": "logloss",
        "scale_pos_weight": 3,
...
trainer_all = CrowdfundingTrainer(
    model_type="xgb",
    params={
        "eval_metric": "logloss",
        "scale_pos_weight": 1,
...
exp = explainer.explain_instance(data_row=instance, predict_fn=trainer_all.model.predict_proba)
